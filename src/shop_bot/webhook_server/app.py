...
def create_webhook_app(bot_controller_instance):
    ...
    @flask_app.route('/payments/approve/<int:document_id>', methods=['POST'])
    @login_required
    def approve_payment_route(document_id):
        ...
        if document['status'] != 'pending':
            flash('Этот документ уже был обработан.', 'warning')
            return redirect(url_for('payments_page'))

        transaction = get_transaction_by_id(document['transaction_id'])
        if not transaction:
            flash('Транзакция не найдена.', 'danger')
            return redirect(url_for('payments_page'))

        # Обновляем статусы
        update_bank_payment_document_status(document_id, 'approved')
        update_transaction_status(document['transaction_id'], 'paid', 'Банковская карта РФ')
        ...
        if bot and loop and loop.is_running():
            ...
            flash('Платеж одобрен и ключ выдан пользователю.', 'success')
        else:
            flash('Бот не запущен. Платеж одобрен, но ключ не выдан.', 'warning')

        return redirect(url_for('payments_page'))

    @flask_app.route('/payments/reject/<int:document_id>', methods=['POST'])
    @login_required
    def reject_payment_route(document_id):
        document = get_bank_payment_document(document_id)
        if not document:
            flash('Документ не найден.', 'danger')
            return redirect(url_for('payments_page'))
        ...
        update_bank_payment_document_status(document_id, 'rejected')
